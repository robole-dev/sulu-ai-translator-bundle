...
import { AITranslatorConfig, AITranslatorToolbarAction, withAITranslatorButton } from "./containers";

// @todo field registry should export these constants
...
initializer.addUpdateConfigHook("sulu_admin", (config, initialized) => {
    if (!initialized) {
        ...
        viewRegistry.add("app.translation_config", AITranslatorConfig);

        // Connect translator toolbar
        formToolbarActionRegistry.add(
            "app.translator_toolbar",
            AITranslatorToolbarAction
        );

        // Override field types
        // @todo This is a bit verbose and should be fixed within fieldRegistry itself
        ...
        fieldRegistry.add(FIELD_TYPE_TEXT_EDITOR, withAITranslatorButton(TextEditor));
    }
});
